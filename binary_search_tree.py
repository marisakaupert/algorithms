...
class BinaryTree():
	...
	def is_bst( self ):
		...
		def checkBST( node, minValue, maxValue ):
			...
			if (node.key < minValue or node.key > maxValue):
				return False
			
			return ( checkBST(node.left, minValue, node.key - 1) and checkBST(node.right, node.key + 1, maxValue) )

		
		if (checkBST(self.root, MIN, MAX)):
			return True
		else:
			return False
