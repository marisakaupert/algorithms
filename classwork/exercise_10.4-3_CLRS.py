...
"""
02/28/2017 - Added the Queue implementation in the code, instead of importing it as an external module
"""
...
class BinaryTree():
	...
	def non_recursive_preorder_walk(self):
		""" 
		Stack-based, pre-order walk.

		CLRS3, exercise 10.4-3 - A straightforward, non-recursive algorithm for a pre-order treewalk
		of the tree: nodes are listed in this order
			* Parent
			* Left
			* Right

		:return: a Python list of the node keys, in the order in which they have been added to the key.
		:rtype: list
		"""
...
class BinaryTree_Walk_UnitTest(unittest.TestCase):
